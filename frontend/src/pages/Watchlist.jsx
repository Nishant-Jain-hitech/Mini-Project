...
const Watchlist = () => {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-black text-white uppercase mb-8">My Watchlist</h1>
      {items.length === 0 ? (
        <p className="text-slate-500">No items saved yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {items.map(item => (
            <div key={item.id} className="bg-slate-900 rounded-2xl p-4 border border-white/5">
              <img src={item.image} className="rounded-xl mb-4" alt={item.title} />
