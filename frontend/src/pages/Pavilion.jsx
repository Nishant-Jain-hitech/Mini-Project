    ...
    const [posts, setPosts] = useState([
        {
            ...
            user: "ViratFan_18",
            time: "2m ago",
            content: "That cover drive in the 3rd over was pure class! Is he the best technician we've ever seen?",
            likes: 142,
            isLiked: false,
            replies: [
                { id: 101, user: "StatsGuru", content: "Technically perfect. His head position was rock solid.", time: "1m ago" }
            ]
        },
        {
            id: 2,
            user: "CricketAnalyst",
            time: "15m ago",
            content: "Statistical deep dive: Why the Adelaide pitch favors spinners on Day 4. Let's discuss in the thread.",
            likes: 89,
            isLiked: false,
            replies: []
        }
    ...
    return (
        <div className="p-4 md:p-10 max-w-4xl mx-auto animate-in fade-in duration-500 relative">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    <h1 className="text-3xl font-black text-white uppercase italic tracking-tighter">The Pavilion</h1>
                    <p className="text-slate-400 text-sm font-medium">Community Hub • 12.4k fans live</p>
                </div>
                <button
                    type="button"
                    onClick={() => { setReplyTo(null); setIsModalOpen(true); }}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl p-6 shadow-2xl">
                        ...
                        {replyTo && <p className="text-slate-500 text-xs mb-4 line-clamp-1 italic">"{replyTo.content}"</p>}
                        ...
                        <form onSubmit={handleSubmit}>
                            <textarea
                                ...
                                placeholder={replyTo ? "Write your reply... (Cmd + Enter)" : "What's the hot take? (Cmd + Enter)"}
                            ...
                            <div className="flex gap-3 justify-end">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2 text-slate-400 font-bold hover:text-white">Cancel</button>
                                <button type="submit" disabled={!newContent.trim()} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-10 py-2 rounded-xl disabled:opacity-50">
                                    {replyTo ? "Reply" : "Post"}
                                </button>
                            </div>
                        </form>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-500 font-black uppercase">{post.time}</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        {post.replies.length > 0 && (
                            <div className="ml-8 space-y-3 border-l-2 border-white/5 pl-6">
                                {post.replies.map((reply) => (
                                    <div key={reply.id} className="bg-white/5 p-4 rounded-xl border border-white/5">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="text-[10px] font-bold text-blue-400">@{reply.user}</span>
                                            <span className="text-[10px] text-slate-600 font-bold uppercase">{reply.time}</span>
                                        </div>
