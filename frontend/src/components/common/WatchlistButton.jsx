  ...
  return (
    <button 
      ...
      className={`p-2 rounded-lg transition-all border border-white/10 flex items-center justify-center ${
        isInWatchlist ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'bg-black/40 text-white/60 hover:bg-white/10'
      }`}
    >
      {isInWatchlist ? (
        <span className="text-xs font-bold flex items-center gap-1">✓ Saved</span>
      ) : (
        <span className="text-xs font-bold flex items-center gap-1">+ Watchlist</span>
      )}
    </button>
  );
};
