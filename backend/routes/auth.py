...
async def update_profile(
    update_data: UserUpdate = Body(...), current_user: dict = Depends(get_current_user)
):
    db = get_db()
    ...
    result = await db.users.update_one(
        {"email": current_user["email"]}, {"$set": update_dict}
    )
...
async def toggle_watchlist(
    movie: MovieItem, current_user: dict = Depends(get_current_user)
):
    db = get_db()
    content_id = movie.id
    ...
    if content_id in current_watchlist_ids:
        await db.users.update_one(
            {"email": current_user["email"]}, {"$pull": {"watchlist": content_id}}
        )
        action = "removed"
    else:
        await db.users.update_one(
            {"email": current_user["email"]}, {"$addToSet": {"watchlist": content_id}}
        )
